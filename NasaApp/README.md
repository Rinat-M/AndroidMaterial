...
10. Переделал удаление задачи из списка по swipe-у, ранее удаление происходило по номеру позиции в списке, но при использовании поиска это было некорректное поведение. Поэтому переделал удаление по id задачи. Предполагается, то id должно быть уникальным.
